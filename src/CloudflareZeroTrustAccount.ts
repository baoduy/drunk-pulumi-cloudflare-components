...
export class CloudflareZeroTrustAccount extends BaseComponent<CloudflareZeroTrustAccountArgs> {
    ...
    private createGatewaySettings() {
        ...
        new ZeroTrustConnectivitySettingsResource(`${this.name}-connectivity-settings`, {
            accountId: this.accountId!,
    ...
    private createDevicePostureRules(intune?: cf.ZeroTrustDevicePostureIntegration) {
        ...
        const intuneRule = intune ? new cf.ZeroTrustDevicePostureRule(
    ...
    private createPrivateApp({name, description, ipAddresses, policies, ports, defaultVnet, tunnel}: PrivateAppType & {
        ...
        if (policies?.requiredDevicePosture && this.devicePosturePolicy)
            policyIds.push(this.devicePosturePolicy.id);
        if (policies?.requiredSso && this.ssoPolicy)
            policyIds.push(this.ssoPolicy.id);
        ...
        return new ZeroTrustPrivateApplication(`${this.name}-${name.replace(/\s+/g, '').toLowerCase()}-private-app`, {
            name,
            ipAddresses,
            description,
            ports,
