import { ipVersion, normalizeIp, parseIp, stringifyIp } from './ip-bigint';
...
function doNormalize(
    cidr: Network,
    { compress = true, hexify = false }: NormalizeOpts = {}
): Network {
    ...
    if (start !== end || prefixPresent) {
        ...
        const ip = normalizeIp(stringifyIp({ number: start, version }), {
            compress,
            hexify,
        });
        return `${ip}/${prefix}`;
...
export function normalizeCidr<T extends Network | Network[]>(
    cidr: T,
    opts?: NormalizeOpts
): T {
...
function subparts(part: Part): Part[] {
    ...
    if (part.end - part.start === 1n) {
        if (part.end % 2n === 0n) {
            return [
                { start: part.start, end: part.start },
                { start: part.end, end: part.end },
            ];
        } else {
...
function formatPart(part: Part, version: IpVersion): CIDR {
    const ip = normalizeCidr(
        stringifyIp({ number: BigInt(part.start.toString()), version })
    );
    const zeroes = diff(part.end, part.start).toString(2);
    const prefix =
        bits[version as ValidIpVersion] - (zeroes.match(/0/g) || []).length;
    return `${ip}/${prefix}`;
}
...
function mapNets(nets: ParsedCidr[]): NetMap {
    ...
    for (const { start, end, version } of nets) {
        if (!maps[version][String(start)])
            maps[version][String(start)] = { start: 0, end: 0 };
        if (!maps[version][String(end)])
            maps[version][String(end)] = { start: 0, end: 0 };
...
export function mergeCidr(nets: Networks): Network[] {
    ...
    const arr: ParsedCidr[] = uniq(
        (Array.isArray(nets) ? nets : [nets]).sort(compare).map(parseCidr)
    );
...
export function excludeCidr(base: Networks, excl: Networks): Network[] {
    const basenets: Network[] = mergeCidr(
        uniq(Array.isArray(base) ? base : [base])
    );
    const exclnets: Network[] = mergeCidr(
        uniq(Array.isArray(excl) ? excl : [excl])
    );
